...
const BrokerChangePlan = () => {
  const navigate = useNavigate();
  const location = useLocation();

  const routeState = location.state as
    | {
        currentPlanId?: SubscriptionPlanKey;
        currentCycle?: BillingCycle;
      }
    | undefined;

  const [cycle, setCycle] = useState<BillingCycle>(
    routeState?.currentCycle ?? "monthly",
  );

  const currentPlanId = routeState?.currentPlanId ?? "professional";
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={() => navigate("/broker-dashboard/subscription")}
...
export default BrokerChangePlan;
